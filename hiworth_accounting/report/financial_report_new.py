...
class FinancialHiworthReportWizard(models.TransientModel):
	...
	def get_stock_lines(self):
		...
		account_obj = self.env['account.account']
	  	...
	  	history = self.env['stock.history'].search([('date','<=',self.date_to),('company_id','=',self.company_id.id)])
	...
	def get_opening_stock(self):
		...
		account_obj = self.env['account.account']
	  	...
	  	# history = self.env['stock.history'].search([('date','<',self.date_from),('company_id','=',self.company_id.id),('product_id.type','=','product')])
		...
		for order in history:
			...
			if order.product_id.type == 'product':
				if len(final_list) == 0:
					list.append({'parent':'Opening Stock', 'amount':order.inventory_value, 'account_list':[{'account':order.product_id.categ_id.name,'amount':order.inventory_value}]})
				if len(final_list) != 0:
					a = list.index(final_list[0])
					list[a]['amount'] += order.inventory_value
					# list[a]['amount'] -= order.credit
	...
	def get_income_lines(self):
		...
		for order in orders:
			# list[list.index(filter(lambda n: n.get('test') == 'test', list)[0])]['total_amount'] -= order.debit
			# list[list.index(filter(lambda n: n.get('test') == 'test', list)[0])]['total_amount'] += order.credit
			# total_list = filter(lambda x: x['test'] == 'test', list)
			# b = list.index(total_list[0])
			# list[b]['total_amount'] -= order.debit
			# list[b]['total_amount'] += order.credit
			final_list = filter(lambda x: x['parent'] == order.account_id.parent_id.name, list)
			if len(final_list) == 0:
				list.append({'parent':order.account_id.parent_id.name, 'amount':order.credit-order.debit, 'account_list':[{'account':order.account_id.name,'amount':order.credit-order.debit}]})
			if len(final_list) != 0:
	...
	@api.multi
	def get_profit_loss_acnt(self):
		opening_stock = closing_stock = expense_amt = income_amt = expense_total = income_total = 0
		if self.target_move == 'posted':
		  orders_income = self.env['account.move.line'].search([('date','>=',self.date_from),('date','<=',self.date_to),('company_id','=',self.company_id.id),('status','=','posted'),('account_type','=','income')])
		  orders_expense = self.env['account.move.line'].search([('date','>=',self.date_from),('date','<=',self.date_to),('company_id','=',self.company_id.id),('status','=','posted'),('account_type','=','expense')])
		else:
			orders_income = self.env['account.move.line'].search([('date','>=',self.date_from),('date','<=',self.date_to),('company_id','=',self.company_id.id),('account_type','=','income')])
			orders_expense = self.env['account.move.line'].search([('date','>=',self.date_from),('date','<=',self.date_to),('company_id','=',self.company_id.id),('account_type','=','expense')])
		for order in orders_income:
			income_amt -= order.debit
			income_amt += order.credit
		for order in orders_expense:
			expense_amt += order.debit
			expense_amt -= order.credit
		  
		open_history = self.env['stock.history'].search([('date','<',self.date_from),('company_id','=',self.company_id.id)])
		for order in open_history:
			opening_stock += order.inventory_value

		closing_history = self.env['stock.history'].search([('date','<=',self.date_to),('company_id','=',self.company_id.id)])
		for order in closing_history:
			closing_stock += order.inventory_value
		expense_total = opening_stock + expense_amt
		income_total = closing_stock + income_amt
