...
class ReservationDetailReport(report_sxw.rml_parse):
    ...
    def _get_room_type(self, reservation_line, categories):
        room_types = ''
        ...
        if not reservation_line:
            for categ in categories:
                room_types += categ.category_id.name
                room_types += ' '
    ...
    def _get_room_nos(self, reservation_line):
        room_nos = ''
        for line in reservation_line:
            for room in line.reserved:
                room_nos += str(room.name.name)
                room_nos += '   '
        return room_nos
    ...
    def get_checkin(self, date_start, date_end):
        ...
        tids = reservation_obj.search(self.cr, self.uid,
                                      [('checkin', '>=', date_start),
                                       ('checkin', '<=', date_end)])
        res = reservation_obj.browse(self.cr, self.uid, tids)
        recordset = res.sorted(key=lambda r: r.checkin)
        return recordset
#         return res
    ...
    def get_checkout(self, date_start, date_end):
        ...
        tids = reservation_obj.search(self.cr, self.uid,
                                      [('checkout', '>=', date_start),
                                       ('checkout', '<=', date_end)])
        res = reservation_obj.browse(self.cr, self.uid, tids)
        recordset = res.sorted(key=lambda r: r.checkout)
        return recordset
#         return res
