...
class HrHolidays(models.Model):
	...
	def _compute_no_days_new(self):
		for record in self:
			...
			if record.leave_id:
				...
				if record.leave_id.name == 'CL':
					record.remaining = record.employee_id.casual_leave
	...
	def action_validate(self):
		...
		for record in self:
			...
			if record.type == 'remove':
				...
				if record.holiday_type == 'employee':
					if record.leave_id.name == 'CL':
						record.employee_id.casual_leave = record.employee_id.casual_leave - record.nos
						record.state = 'validate'
	...
	def approve_leave(self):
		view_id = self.env.ref('hiworth_attendance.view_wizard_approve_lop').id
		return {
			'name':'Loss Of Pay',
			'view_type':'form',
			'view_mode':'form',
