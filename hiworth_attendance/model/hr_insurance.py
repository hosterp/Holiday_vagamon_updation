...
class ManagementPolicy(models.Model):
	...
	def domain_res_company_id(self):
		employee_pool = self.env['hr.employee']
		employee_ids = employee_pool.search([('manager','=',True)])
		partner_list  = []
		for emp in employee_ids:
			partner_list.append(emp.user_id.partner_id.id)
			
		return {'domain':{'res_company_id':[('id','in',partner_list)]}}

	# name = fields.Char('Policy')
	name = fields.Char('Name', compute="_get_name")
	res_company_id = fields.Many2one('res.partner', string='Policy Holder')
