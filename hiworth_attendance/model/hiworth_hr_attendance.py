...
class HiworthHrAttendance(models.Model):
    ...
    def create(self, vals):
        ...
        if datetime.now().weekday() == 6:
            ...
            if self.env['hr.employee'].browse(vals.get('name')).casual_leave>1:
                present_work +=1
                self.env['hr.employee'].browse(vals.get('name')).casual_leave = self.env['hr.employee'].browse(vals.get('name')).casual_leave -1
            if present_work >= 3:
